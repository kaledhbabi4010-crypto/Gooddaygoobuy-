...
# 2. Test Wikipedia Cloud API connectivity (used by Gateway 2)
...
assert len(results) > 0, 'Wikipedia Cloud API search returned no results!'
print('  Cloud API Success:', results[0]['title'])
"
